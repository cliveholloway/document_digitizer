...
def main(input_dir, output_dir, config, verbose):
    ...
    logger.info(f"Deskew config: angle_threshold={config_data['deskew']['angle_threshold']}°, max_rotation={config_data['deskew']['max_rotation']}°")
    ...
    summary = ", ".join([f"{count} {ext}" for ext, count in sorted(extension_counts.items())])
    ...
    for i, image_file in enumerate(image_files, 1):
        ...
        try:
            ...
            elapsed_time = time.time() - start_time
            error_msg = f"✗ Error processing {image_file.name} after {elapsed_time:.2f}s: {e}"
...
def deskew_image(img_array, processing_config):
    ...
    if abs(detected_angle) < angle_threshold:
        logger.debug(f"    Angle {detected_angle:.3f}° < threshold {angle_threshold}°. Skipping rotation.")
        return img_array, detected_angle, 0
    
    if abs(detected_angle) > max_rotation:
        logger.debug(f"    Angle {detected_angle:.3f}° > max rotation {max_rotation}°, skipping to avoid overcorrection.")
        return img_array, detected_angle, 0
    ...
    if angle_rad < np.pi/4:  # Less than 45 degrees
        ...
        if crop_y > 0:
            ...
            cropped = rotated[crop_y:h-crop_y, :]  # Only crop top/bottom, keep full width
            logger.debug(f"    ✓ Rotated by {detected_angle:.3f}° and cropped {crop_y}px from top/bottom")
            logger.debug(f"    ✓ Final size: {w}x{h} → {cropped.shape[1]}x{cropped.shape[0]}")
...
def detect_rotation_angle(gray, timeout_seconds=10):
    ...
    if len(methods_tried) == 1:
        ...
        logger.info(f"    Using median of {len(reasonable_angles)} reasonable results: {final_angle:.3f}°")
...
def process_hough_lines(lines, method_name):
    ...
    logger.debug(f"      Found {len(angles)} valid lines with {method_name} method (from {len(lines_to_process)} total)")
    ...
    if len(angles) >= 2:
        ...
        logger.info(f"      {method_name.capitalize()} result: {final_median:.3f}° from {len(filtered_angles)} lines")
    ...
    try:
        ...
        for threshold in [40, 25, 15]:
            ...
            if lines is not None and len(lines) >= 3:
                logger.debug(f"      Standard Hough found {len(lines)} lines with threshold {threshold}")
                ...
                if len(angles) >= 2:
                    ...
                    logger.debug(f"      Standard result: {median_angle:.3f}° from {len(filtered_angles)} lines")
                    return median_angle
...
def try_projection_method_simple(gray):
    ...
    try:
        ...
        for angle in angles_to_try:
            ...
            rotation_matrix = cv2.getRotationMatrix2D((small_w//2, small_h//2), angle, 1.0)
            rotated = cv2.warpAffine(small_gray, rotation_matrix, (small_w, small_h), borderValue=255)
        ...
        logger.debug(f"      Best projection angle: {best_angle:.3f}° (score: {best_score:.0f})")
