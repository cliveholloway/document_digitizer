...
def find_overlap_between_images(top_img, bottom_img, config):
    ...
    top_gray = (
        cv2.cvtColor(top_img, cv2.COLOR_BGR2GRAY)
        if len(top_img.shape) == 3
        else top_img
    )
    bottom_gray = (
        cv2.cvtColor(bottom_img, cv2.COLOR_BGR2GRAY)
        if len(bottom_img.shape) == 3
        else bottom_img
    )
...
def create_stitched_image(top_img, bottom_img, y_overlap, x_offset):
    ...
    stitched[
        bottom_y_start : bottom_y_start + bottom_h,
...
def concatenate_images(top_img, bottom_img):
    ...
    if top_img.shape[1] != target_width:
        top_resized = cv2.resize(
            top_img, (target_width, top_img.shape[0]), interpolation=cv2.INTER_CUBIC
        )
    else:
        top_resized = top_img
    ...
    if bottom_img.shape[1] != target_width:
        bottom_resized = cv2.resize(
            bottom_img,
            (target_width, bottom_img.shape[0]),
